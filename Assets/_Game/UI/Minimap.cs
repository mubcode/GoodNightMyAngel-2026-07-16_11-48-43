...
namespace GoodNightMyAngel.UI
{
    ...
    public class Minimap : MonoBehaviour
    {
        ...
        private void OnGUI()
        {
            ...
            // Yollar (PathManager - EnemyRoute sistemi)
            if (PathManager.Instance != null && PathManager.Instance.RouteCount > 0)
            {
                GUI.color = pathColor;
                for (int p = 0; p < PathManager.Instance.RouteCount; p++)
                {
                    var route = PathManager.Instance.GetRoute(p);
                    if (route == null || route.points == null || route.points.Count < 2) continue;
                    for (int i = 0; i < route.points.Count - 1; i++)
                    {
                        Vector2 a = WorldToMap(route.points[i], center, scale, x, y);
                        Vector2 b = WorldToMap(route.points[i + 1], center, scale, x, y);
                        DrawLine(a, b, 1.5f);
                    }
                    // Yatak hattı (her rota -> yatak)
                    if (GameManager.Instance.bed != null)
                    {
                        Vector2 lastPoint = WorldToMap(route.points[route.points.Count - 1], center, scale, x, y);
                        Vector2 bedPoint = WorldToMap(GameManager.Instance.bed.transform.position, center, scale, x, y);
                        DrawLine(lastPoint, bedPoint, 1f);
                    }
                }
            }
