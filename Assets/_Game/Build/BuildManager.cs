...
namespace GoodNightMyAngel.Build
{
    ...
    public class BuildManager : MonoBehaviour
    {
        ...
        private GameObject _lockCellObj;            // son geçerli hücre kırmızı göstergesi (build phase sırasında)
        private Material _ghostMaterial;
        private Material _hoverSquareMaterial;
        private Material _lockCellMaterial;
        private Renderer _ghostRenderer;
        private Renderer _hoverSquareRenderer;
        private Renderer _lockCellRenderer;
        ...
        private void OnDestroy()
        {
            ...
            if (_lockCellObj != null) Destroy(_lockCellObj);
        }
        ...
        private void OnDisable()
        {
            ...
            if (_lockCellObj != null) _lockCellObj.SetActive(false);
        }
        ...
        private void Update()
        {
            if (!IsBuildPhase)
            {
                ...
                if (_lockCellObj != null && _lockCellObj.activeSelf) _lockCellObj.SetActive(false);
                return;
            }
        ...
        private void CreatePreviewObjects()
        {
            ...
            // Son geçerli hücre göstergesi (sadece build phase sırasında aktif)
            _lockCellObj = GameObject.CreatePrimitive(PrimitiveType.Quad);
            _lockCellObj.name = "LockCellIndicator";
            var col3 = _lockCellObj.GetComponent<Collider>();
            if (col3 != null) Destroy(col3);
            _lockCellObj.transform.rotation = Quaternion.Euler(90, 0, 0);
            _lockCellObj.SetActive(false);
            _lockCellRenderer = _lockCellObj.GetComponent<Renderer>();
            _lockCellMaterial = new Material(Shader.Find("Universal Render Pipeline/Unlit"));
            _lockCellMaterial.SetFloat("_Surface", 1);
            _lockCellMaterial.SetFloat("_Blend", 0);
            _lockCellMaterial.SetOverrideTag("RenderType", "Transparent");
            _lockCellMaterial.SetInt("_SrcBlend", (int)UnityEngine.Rendering.BlendMode.SrcAlpha);
            _lockCellMaterial.SetInt("_DstBlend", (int)UnityEngine.Rendering.BlendMode.OneMinusSrcAlpha);
            _lockCellMaterial.SetInt("_ZWrite", 0);
            _lockCellMaterial.renderQueue = 3000;
            _lockCellRenderer.sharedMaterial = _lockCellMaterial;
        }

        private void UpdateHoverPreview()
        {
            // Son geçerli hücre kilit göstergesi (sadece build phase'te)
            if (lockFinalApproachCell && _lastValidCell.HasValue)
            {
                Vector3 lockPos = CellToWorld(_lastValidCell.Value) + Vector3.up * 0.015f;
                _lockCellObj.transform.position = lockPos;
                _lockCellObj.transform.localScale = new Vector3(cellSize * 0.97f, cellSize * 0.97f, 1f);
                _lockCellMaterial.color = lockedCellColor;
                if (!_lockCellObj.activeSelf) _lockCellObj.SetActive(true);
            }
            else if (_lockCellObj != null && _lockCellObj.activeSelf)
            {
                _lockCellObj.SetActive(false);
            }
